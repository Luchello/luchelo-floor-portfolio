...
export default function Services() {
  return (
    <section id="services" className="py-24 px-6 bg-cream-50">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-14">
          <span className="text-accent font-medium text-sm tracking-wider">SERVICES</span>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {services.map((svc) => (
            <div
              key={svc.title}
              className="group bg-white rounded-2xl p-8 border border-cream-200/80 shadow-sm hover:shadow-md hover:border-accent/20 transition-all"
            >
              ...
              <p className="text-dark-500 leading-relaxed mb-4">{svc.desc}</p>
              <div className="flex flex-wrap gap-2">
                {svc.tags.map(tag => (
                  <span key={tag} className="bg-cream-100 text-dark-600 text-xs font-medium px-3 py-1 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
